...
const UserCircleSolidIcon = (props = { classList: 'w-6 h-6'}) => x`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="${props.classList}">
  <path fill-rule="evenodd" d="M18.685 19.097A9.723 9.723 0 0 0 21.75 12c0-5.385-4.365-9.75-9.75-9.75S2.25 6.615 2.25 12a9.723 9.723 0 0 0 3.065 7.097A9.716 9.716 0 0 0 12 21.75a9.716 9.716 0 0 0 6.685-2.653Zm-12.54-1.285A7.486 7.486 0 0 1 12 15a7.486 7.486 0 0 1 5.855 2.812A8.224 8.224 0 0 1 12 20.25a8.224 8.224 0 0 1-5.855-2.438ZM15.75 9a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z" clip-rule="evenodd" />
</svg>`;

const ChevronRightIcon = (props = { classList: 'w-6 h-6'}) => x`<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="${props.classList}">
			...
			<header class="h-10 px-5 flex items-center mb-5">
				...
				<div class="ms-auto empty:size-8 empty:bg-slate-300 empty:rounded-full">${
					user
						? x `<a href="#me">${
                                    user.photoURL
                                        ? x`<img src="${user.photoURL}" class="size-8 rounded-full">`
                                        : x`<span class="text-slate-400">${UserCircleSolidIcon({ classList: 'size-8' })}</span>`
                                }</a>`
						: x`<a href="#login">${UserCircleOutlineIcon({classList: 'size-8'})}</a>`
				}</div>
			</header>
				...
				<div>
					<div class="size-12 empty:rounded-full empty:bg-slate-200">${
						user?.photoURL
							...
							: x`<span class="text-slate-400">${UserCircleSolidIcon({ classList: 'size-12' })}</span>`
					}</div>
				</div>
...
function registerStorage() {
    _registerComponent(new Component(STORAGE_TYPE, factory$1, "PUBLIC" /* ComponentType.PUBLIC */).setMultipleInstances(true));
...
const EditProfile = async () => {
    ...
    function uploadPhoto(e) {
        ...
        resizeImage(file, 300, 300)
            .then(data => {
                ...
                const storageRef = ref(storage, `users/profile/${dayjs().format('YYYYMMDD_HHmmss_SSS')}_${currentUser.uid}.${ext}`);
                    ...
                    .then(() => {
                        getDownloadURL(storageRef)
                            .then((url) => {
                                ...
                                j$1(
                                    x`<img src="${url}" class="rounded-full w-full h-full block">`,
    ...
    function deletePhoto() {
        document.querySelector('#profile-preview img')?.remove();
        j$1(
            x`<span class="text-slate-400">${ UserCircleSolidIcon({ classList: 'size-32' }) }</span>`,
                    ...
                    <div id="profile-preview" class="block mx-auto w-32 h-32 empty:bg-slate-200 rounded-full overflow-hidden">${
                        currentUser
                            ? currentUser.photoURL
                                ? x`<img src=${currentUser.photoURL} class="rounded-full w-full h-full block">`
                                : x`<span class="text-slate-400">${ UserCircleSolidIcon({ classList: 'size-32' }) }</span>`
                            : x`${T}`
                    }</div>
